...
#    tf.reset_default_graph()
    ...
    for i in range(0, 5):
        ...
        # # compute posterior
        # f_posterior_out_mu_pred3, f_posterior_out_sigma_pred3 = f_posterior(tf.concat(values=(lstm_state[1], encoder(tf.divide(x=x_[:,i,:,:,:],y=255.0))), axis=1))

        # if i == 0:
        #   z_pred3 = f_posterior_out_mu_pred3 + f_posterior_out_sigma_pred3 * tf.random_normal(shape=[256], mean=0.0, stddev=1.0)
        #   f_z_out_pred3 = f_z(z_pred3)
        # else:
        #   z_pred3 = f_prior_out_mu_pred3 + f_prior_out_sigma_pred3 * tf.random_normal(shape=[256], mean=0.0, stddev=1.0)
        #   f_z_out_pred3 = f_z(z_pred3)
...
#    saver.restore(sess, "/home/aarongu/Desktop/TrainEpoch-810/epoch-810")
    ...
    fuck = sess.run(reconstruction_loss, feed_dict={x_: input[:, :, :, :, :], l_: l[:, :]})
    print('3')
    # fuck = sess.run(tf.divide(x=x_[:,5,:,:,:],y=255.0), feed_dict={x_: input[:,:,:,:,:], l_: l[:, :]})
    sess.close()
    print('4')
...
# wtf = vrnn_prediction(np.zeros((1,5,64,64,3)))
# print(wtf.shape)
