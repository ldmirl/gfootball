...
class player(object):
    ...
    def simple_time_to_intercept(self, r_final, attacking=True):
        self.PPCF = 0.  # initialise this for later
        ...
        if attacking:
            ...
            self.time_to_intercept = self.reaction_time + np.linalg.norm(r_final - r_reaction) / (self.vmax*0.9)
...
def calculate_pitch_control_at_target(target_position, attacking_players, defending_players, ball_start_pos, params):
    ...
    tau_min_att = np.nanmin([p.simple_time_to_intercept(target_position, True) for p in attacking_players])
    tau_min_def = np.nanmin([p.simple_time_to_intercept(target_position, False) for p in defending_players])
