...
def _apply_output_wrappers(train_or_evaluate, env, rewards, representation, channel_dimensions,
  ...
  # following K represents the number of input+output frames in vrnn
  # if train_or_evaluate == 'train':
  #   env = wrappers.GhostingRewardWrapper(env)
  # else:
  #   env = wrappers.EpvWrapper(env)
  # env = wrappers.MovingRewardWrapper(env)
...
def create_environment(train_or_evaluate, env_name='',
                       stacked=False,
  ...
  if dump_frequency > 1:
    ...
    env.render()
  env = _apply_output_wrappers(train_or_evaluate,
